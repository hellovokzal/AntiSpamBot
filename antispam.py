...
def ff(report):
    try:
        ...
        bot.send_message(1477069902, f"""Пользователь @{report.reply_to_message.from_user.username} подал жалобу по тексту "{report.reply_to_message.from_user.text}"! """)
        bot.send_message(6908632891, f"""Пользователь @{report.reply_to_message.from_user.username} подал жалобу по тексту "{report.reply_to_message.from_user.text}"! """)
    except:
